...
st.set_page_config(page_title="Reframe", page_icon="\U0001F4F1", layout="wide",
...
_DEFAULTS = dict(
    ...
    # v5.0: Enhanced panel
    panel_layout_mode="equal",
    panel_speaker_focus_ratio=0.60,
    panel_head_normalize=False,
    panel_lower_third_aware=False,
    panel_portrait_mode=False,
    panel_max_slots=4,
)
...
<div class="rf-top">
 ...
 <span class="rf-name">Reframe</span></div>
...
with mc1:
    if st.button("\U0001F4F1  Single Clip", type="secondary", use_container_width=True):
        st.session_state.app_mode = "single"
with mc2:
    if st.button("\U0001F3AC  Auto-Clip  \u2726", type="secondary", use_container_width=True):
        st.session_state.app_mode = "autoClip"

app_mode = st.session_state.app_mode
if app_mode == "single":
    st.markdown("""
 <div class="rf-mode-box acc"><span style="font-size:16px">\U0001F4F1</span><div>
 <div class="rf-mode-h acc">Single Clip</div>
 <div class="rf-mode-s">Upload any landscape video. AI tracks your subject and converts to 9:16 in one pass.
Subject tracking (YOLOv8) or face-locked Talking Head mode.</div></div></div>
 """, unsafe_allow_html=True)
else:
    st.markdown("""
 <div class="rf-mode-box pur"><span style="font-size:16px">\U0001F3AC</span><div>
 ...
 <div class="rf-mode-s">Upload a 30\u201390 min video. AI scans for saliency peaks, detects narrative arcs, identifies the SOI coordinate region per clip, enforces the lower-third safe zone, then verticalizes every selected clip.</div></div></div>
 """, unsafe_allow_html=True)
...
with tm1:
    if st.button("\U0001F3AF  Subject Tracking", type="secondary", use_container_width=True):
        st.session_state.tracking_mode = "subject"
with tm2:
    if st.button("\U0001F464  Talking Head  \u2726", type="secondary", use_container_width=True):
        st.session_state.tracking_mode = "talking_head"
st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
if st.button("\U0001F3C0  Sports Action  \u2726  (Ball-aware \u00b7 Kalman)", type="secondary", use_container_width=True):
...
if tracking_mode == "sports_action":
    ...
    st.markdown(f'<div style="display:flex;align-items:center;gap:8px;margin-top:6px;margin-bottom:4px;"><span style="background:var(--acc);color:#fff;font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:3px 10px;border-radius:99px;">\U0001F3C0 Sports Action \u00b7 {sd}</span><span style="font-size:11px;color:var(--ink3);">Ball-aware \u00b7 Kalman tracking</span></div>', unsafe_allow_html=True)
elif tracking_mode == "talking_head":
    st.markdown('<div style="margin-top:6px;margin-bottom:4px;"><span style="background:var(--pur);color:#fff;font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:3px 10px;border-radius:99px;">\U0001F464 Talking Head</span></div>', unsafe_allow_html=True)
else:
    st.markdown('<div style="margin-top:6px;margin-bottom:4px;"><span style="background:var(--ink);color:#fff;font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:3px 10px;border-radius:99px;">\U0001F3AF Subject Tracking</span></div>', unsafe_allow_html=True)

# ── v5.0: Enhanced Panel Mode Controls (Subject Tracking only) ──
if tracking_mode == "subject":
    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    with st.expander("\U0001F39B Panel Mode Settings", expanded=False):
        ...
        panel_mode_override = st.radio("Panel mode", ["auto","force_on","force_off"],
            format_func=lambda x: {"auto":"\U0001F916 Auto-detect (recommended)","force_on":"\u2705 Force ON","force_off":"\u274C Force OFF"}[x],
        ...
        if panel_mode_override != "force_off":
            ...
            # ── v5.0: Layout mode ──
            layout_mode = st.radio("Layout mode",
                ["equal", "speaker_focus", "solo_spotlight", "auto"],
                format_func=lambda x: {
                    "equal": "\u2796 Equal Split (same size for all)",
                    "speaker_focus": "\U0001F50A Speaker Focus (60/40 for active speaker)",
                    "solo_spotlight": "\U0001F4A1 Solo Spotlight (single person fills frame)",
                    "auto": "\U0001F916 Auto (switches based on audio energy)",
                }[x],
                index=["equal","speaker_focus","solo_spotlight","auto"].index(
                    st.session_state.get("panel_layout_mode","equal")),
            ...
            if layout_mode == "speaker_focus":
                st.session_state.panel_speaker_focus_ratio = st.slider(
                    "Speaker focus ratio", 0.50, 0.80,
                    float(st.session_state.get("panel_speaker_focus_ratio", 0.60)), 0.05,
                    format="%.2f",
                    help="How much space the active speaker gets (0.6 = 60%)")

            # ── v5.0: Feature toggles ──
            fc1, fc2 = st.columns(2)
            with fc1:
                st.session_state.panel_head_normalize = st.toggle(
                    "\U0001F9D1 Equal head sizing", value=st.session_state.get("panel_head_normalize", False),
                    help="Detect faces and normalize crop scale so all faces are ~same pixel height")
                st.session_state.panel_lower_third_aware = st.toggle(
                    "\U0001F4CB Lower-third awareness", value=st.session_state.get("panel_lower_third_aware", False),
                    help="Detect text banners and avoid cropping into them")
            with fc2:
                st.session_state.panel_portrait_mode = st.toggle(
                    "\U0001F5BC Portrait extraction", value=st.session_state.get("panel_portrait_mode", False),
                    help="Use face landmarks to crop head-and-shoulders, not full body")
                st.session_state.panel_max_slots = st.slider(
                    "Max persons", 2, 4, int(st.session_state.get("panel_max_slots", 4)), 1,
                    help="Maximum number of people to track in panel mode")
        ...
        if panel_mode_override == "auto":
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.session_state.panel_max_motion = st.slider("Max motion (px)", 5.0, 40.0,
                    float(st.session_state.get("panel_max_motion", 20.0)), 1.0,
                    help="Higher = allow more movement")
                st.session_state.panel_min_area = st.slider("Min person area (%)", 0.01, 0.10,
                    float(st.session_state.get("panel_min_area", 0.03)), 0.01, format="%.2f",
                    help="Lower = detect smaller people in wide shots")
            with c2:
                st.session_state.panel_max_variance = st.slider("Max count variance", 0.5, 5.0,
                    float(st.session_state.get("panel_max_variance", 2.5)), 0.5,
                    help="Higher = tolerate people entering/exiting frame")
                st.session_state.panel_stability = st.slider("Stability fraction", 0.30, 0.90,
                    float(st.session_state.get("panel_stability", 0.60)), 0.05, format="%.2f",
                    help="Lower = require less consistent positioning")

# Settings tabs
tab_list = ["\U0001F39E Output", "\U0001F3AF Tracking", "\U0001F4DD Subtitles", "\u2699 Advanced"]
if app_mode == "autoClip":
    tab_list += ["\u2702\uFE0F Clips", "\U0001F4CA Analytics"]
if app_mode == "autoClip":
...
with tab_trk:
    ...
    if tracking_mode == "talking_head":
        ...
        st.markdown('<div class="rf-info" style="margin-bottom:12px;">\U0001F3C0 <b>Sports Mode Active</b> \u2014 Ball-aware tracking with Kalman predictive smoothing.</div>', unsafe_allow_html=True)
...
with tab_sub:
    ...
    with s1:
        ...
        translate_subtitles = st.toggle("Translate \U0001F310", value=False, disabled=(not _whisper_ok or not _translate_ok or not burn_subtitles))
...
if app_mode == "autoClip" and tab_clip is not None:
    with tab_clip:
        ...
        with cl1:
            preset_label = st.radio("Clip length preset", list(_CLIP_PRESETS.keys()), index=2, help="Sets the target duration window")
        ...
        st.markdown('<div class="rf-info" style="margin-top:8px;">\U0001F4A1 AI detects saliency peaks + scene boundaries to find narrative arcs.</div>', unsafe_allow_html=True)
    ...
    with tab_analytics:
        ...
        if st.session_state.clip_results:
            ...
            if valid_results:
                ...
                st.markdown(f'<div class="rf-analytics"><div class="rf-an-title">\U0001F4CA Batch Analytics</div><div class="rf-an-grid"><div class="rf-an-item"><div class="rf-an-label">Total Input</div><div class="rf-an-val">{total_in:.1f} MB</div></div><div class="rf-an-item"><div class="rf-an-label">Total Output</div><div class="rf-an-val good">{total_out:.1f} MB</div><div class="rf-an-sub">{((1-total_out/total_in)*100):.1f}% smaller</div></div><div class="rf-an-item"><div class="rf-an-label">Avg Compression</div><div class="rf-an-val">{avg_ratio:.2f}x</div></div></div></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="rf-info">No analytics data available yet.</div>', unsafe_allow_html=True)
        else:
...
# ─── Helper: build PanelModeConfig from session state (DRY) ────────────────
def _build_panel_config() -> PanelModeConfig:
    return PanelModeConfig(
...
with col_src:
    ...
    if uploaded_file is not None and st.session_state.input_path:
        ...
        st.markdown(f'<div class="rf-chip"><span>\U0001F3AC</span><strong>{uploaded_file.name}</strong><span style="color:var(--bdr2)">\u00b7</span><span>{mb_str}</span></div>', unsafe_allow_html=True)
    ...
    if app_mode == "single":
        ...
        if st.session_state.processing_done and st.session_state.output_bytes:
            ...
            if st.session_state.srt_bytes:
                st.download_button("\u2193  Download subtitles (.srt)", data=st.session_state.srt_bytes, file_name=f"{stem}.srt", mime="text/plain", use_container_width=True)
        else:
            st.markdown('<div class="rf-empty"><div class="rf-empty-icon">\U0001F4F1</div><div class="rf-empty-h">Vertical output</div><div class="rf-empty-s">appears here after conversion</div></div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="rf-sec">Detected Clips</div>', unsafe_allow_html=True)
        if not st.session_state.scan_done:
            st.markdown('<div class="rf-empty"><div class="rf-empty-icon">\U0001F50D</div><div class="rf-empty-h">Scan first</div><div class="rf-empty-s">AI will detect high-engagement segments</div></div>', unsafe_allow_html=True)
            ...
            if st.session_state.clip_results:
                for r in st.session_state.clip_results:
                    co = r.get("clip")
                    if co: clip_results_map[(round(co.start_sec,1), round(co.end_sec,1))] = r
            pi = st.session_state.playing_clip_idx
            for ci, clip in enumerate(clips):
                sp = int(clip.score*100); sc = "h" if clip.score>0.7 else ("m" if clip.score>0.4 else "")
                is_s = ci in sel; is_p = pi == ci
                ck = (round(clip.start_sec,1), round(clip.end_sec,1)); rfc = clip_results_map.get(ck)
                ...
                st.markdown(f'<div class="{cc}"><span class="rf-cscore {sc}">{sp}%</span><div class="rf-ctitle">Clip {ci+1}</div><div class="rf-cmeta">{ts}</div><span class="rf-cdur">{clip.duration:.0f}s</span><span class="rf-csoi">SOI: {clip.soi_region}</span>{dt}</div>', unsafe_allow_html=True)
                if is_d:
            ...
            st.markdown('<div class="rf-empty"><div class="rf-empty-icon">\U0001F50D</div><div class="rf-empty-h">No clips found</div><div class="rf-empty-s">try adjusting clip duration</div></div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
        ...
        if not st.session_state.processing_done:
            ...
            if go:
                ...
                try:
                    ...
                    else: status.error("Output is empty \u2014 check your FFmpeg installation.")
            ...
            with r2:
                if info and st.session_state.output_bytes:
                    in_mb = len(uploaded_file.getvalue())/(1024**2); out_mb = len(st.session_state.output_bytes)/(1024**2)
                    delta = out_mb - in_mb; dc = "var(--grn)" if delta<0 else "var(--acc)"
                    st.markdown(f"<p style='color:var(--ink3);font-size:11px;text-align:right;margin-top:12px;'>{out_mb:.1f} MB <span style='color:{dc}'>({delta:+.1f} MB)</span></p>", unsafe_allow_html=True)
    else:
        if st.session_state.detected_clips is None: st.session_state.scan_done = False
        if not st.session_state.scan_done:
            b1, b2, b3 = st.columns([4,4,2])
            with b1: scan_btn = st.button("\U0001F50D  Scan for Clips", type="primary", use_container_width=True, disabled=not can_go)
            with b2:
                if info:
                    dur=info["duration_seconds"]; est=max(10,dur*0.05); est_s=f"~{int(est//60)}m {int(est%60):02d}s" if est>=60 else f"~{int(est)}s"
                    st.markdown(f"<p style='color:var(--ink3);font-size:11px;margin-top:12px;'>Scan est. {est_s}</p>", unsafe_allow_html=True)
            with b3:
                if st.button("Clear", type="secondary", use_container_width=True): _cleanup(); st.session_state.uploaded_file_name=None; st.rerun()
            if scan_btn:
                prog=st.progress(0.0); status=st.empty(); status.info("\U0001F50D Scanning\u2026")
                try:
                ...
                except Exception as exc: status.error(f"Scan error: {exc}"); import traceback; print(traceback.format_exc())
        else:
            ...
            if not clips:
                ...
                if st.button("\U0001F504 Re-scan", type="secondary"): st.session_state.scan_done=False; st.session_state.detected_clips=None; st.rerun()
                st.stop()
            if st.session_state.selected_clip_indices is None: st.session_state.selected_clip_indices = set(range(len(clips)))
            sel = st.session_state.selected_clip_indices
            if not st.session_state.clip_results:
                ...
                with p2:
                    if st.button("\U0001F504 Re-scan", type="secondary", use_container_width=True): st.session_state.scan_done=False; st.session_state.detected_clips=None; st.session_state.clip_results=None; st.rerun()
                with p3:
                    if st.button("Clear", type="secondary", use_container_width=True): _cleanup(); st.session_state.uploaded_file_name=None; st.rerun()
                if pb and sel:
                    ...
                    except Exception as exc: status.error(f"Error: {exc}"); import traceback; print(traceback.format_exc())
            else:
                results = st.session_state.clip_results; nk = sum(1 for r in results if not r.get("error"))
                if clips: st.markdown(f'<div class="rf-ok">\u2713 {nk} clip{"s" if nk!=1 else ""} ready \u2014 download from the cards above</div>', unsafe_allow_html=True)
                ...
                with rc3:
                    if st.button("\U0001F5D1 Clear cache", type="secondary", use_container_width=True): _cleanup(); st.session_state.uploaded_file_name=None; st.cache_data.clear(); st.rerun()
else:
    st.markdown("""
