...
def _render_panel_frame(frame: np.ndarray, persons: List[Tuple], out_w: int, out_h: int,
    ...
    if layout == "auto":
        if n == 1: layout = "solo_spotlight"
        elif audio_rms_per_person and max(audio_rms_per_person) > 2.0 * (sum(audio_rms_per_person)/max(len(audio_rms_per_person),1)):
            layout = "speaker_focus"
        else: layout = "equal"
    n_slots = min(max(n, 2), cfg.max_slots)
    if n == 0:
        groups = [prev_slots[si] if prev_slots and si < len(prev_slots) else [] for si in range(n_slots)]
    elif n == 1:
        groups = [persons]
        while len(groups) < n_slots:
            groups.append(prev_slots[len(groups)] if prev_slots and len(groups) < len(prev_slots) else persons)
    else:
        sorted_p = sorted(persons, key=lambda p: (p[0]+p[2])//2)
        if n <= n_slots:
            groups = [[p] for p in sorted_p]
            while len(groups) < n_slots: groups.append([])
        else:
            chunk = max(1, n // n_slots); groups = []
            for si in range(n_slots):
                s = si * chunk; e = s + chunk if si < n_slots-1 else n
                groups.append(sorted_p[s:e])
    if slot_smoother is not None:
        if len(groups) == 2:
            groups[0], groups[1] = slot_smoother.update(groups[0], groups[1], strip_w=float(out_w))
        else:
            sm = slot_smoother.update(*groups, strip_w=float(out_w))
            groups = list(sm) if isinstance(sm, (list, tuple)) else groups
    canvas = np.empty((out_h, out_w, 3), dtype=np.uint8)
    active_slots = [si for si in range(n_slots) if groups[si]]
    na = max(len(active_slots), 1)
    if orientation == "horizontal":
        if layout == "speaker_focus" and audio_rms_per_person and na >= 2:
            heights = [int(out_h * (1-cfg.speaker_focus_ratio) / max(na-1,1))] * na
            se = [audio_rms_per_person[si] if si < len(audio_rms_per_person) else 0.0 for si in active_slots]
            loud = se.index(max(se)) if se else 0
            heights[loud] = int(out_h * cfg.speaker_focus_ratio)
            rem = out_h - heights[loud]
            for i in range(na):
                if i != loud: heights[i] = rem // max(na-1, 1)
        elif layout == "solo_spotlight" and na >= 1:
            heights = [out_h]; active_slots = active_slots[:1]; na = 1
        else:
            heights = [out_h // na] * na
        ...
        for idx, si in enumerate(active_slots):
            h = heights[idx]
            strip = _crop_group_to_strip(frame, groups[si], out_w, h,
                vignette_strength=vignette_strength, color_grade=color_grade,
                portrait_mode=cfg.portrait_mode, lower_third_y=lower_third_y)
            canvas[yo:yo+h, :] = strip; yo += h
            if idx < na - 1:
                d1 = max(0, yo - PANEL_DIVIDER_PX//2); d2 = min(out_h, yo + (PANEL_DIVIDER_PX+1)//2)
                canvas[d1:d2, :] = PANEL_DIVIDER_COLOR
    else:
        if layout == "speaker_focus" and audio_rms_per_person and na >= 2:
            widths = [int(out_w * (1-cfg.speaker_focus_ratio) / max(na-1,1))] * na
            se = [audio_rms_per_person[si] if si < len(audio_rms_per_person) else 0.0 for si in active_slots]
            loud = se.index(max(se)) if se else 0
            widths[loud] = int(out_w * cfg.speaker_focus_ratio)
            rem = out_w - widths[loud]
            for i in range(na):
                if i != loud: widths[i] = rem // max(na-1, 1)
        else:
            widths = [out_w // na] * na
        ...
        for idx, si in enumerate(active_slots):
            w = widths[idx]
            strip = _crop_group_to_strip(frame, groups[si], w, out_h,
                vignette_strength=vignette_strength, color_grade=color_grade,
                portrait_mode=cfg.portrait_mode, lower_third_y=lower_third_y)
            canvas[:, xo:xo+w] = strip; xo += w
            if idx < na - 1:
                d1 = max(0, xo - PANEL_DIVIDER_PX//2); d2 = min(out_w, xo + (PANEL_DIVIDER_PX+1)//2)
                canvas[:, d1:d2] = PANEL_DIVIDER_COLOR
