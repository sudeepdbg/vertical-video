...
def _ffmpeg_encode(video_path: str, audio_source: Optional[str], output_path: str,
    ...
    # Robust input flags for MJPEG/AVI or MP4 intermediate files
    cmd = ["ffmpeg", "-y",
           "-fflags", "+genpts",
           "-probesize", "50M",
           "-analyzeduration", "10M",
    ...
    if subtitle_path and os.path.exists(subtitle_path):
        ...
        fs, pc, oc, ol = style.get("fontsize", 18), style.get("primary_color", "&H00FFFFFF"), style.get("outline_color", "&H00000000"), style.get("outline", 2)
        bold, shad, bc, mv = style.get("bold", 1), style.get("shadow", 0), style.get("back_color", "&H00000000"), style.get("margin_v", 80)
        srt_esc = subtitle_path.replace("\\", "/").replace(":", "\\:")
        force_style = f"Fontsize={fs},PrimaryColour={pc},OutlineColour={oc},Outline={ol},Bold={bold},Shadow={shad},BackColour={bc},MarginV={mv},Alignment=2"
...
def get_video_info(path: str) -> Dict[str, Any]:
    ...
    cap.release()
    return {"fps": fps, "total_frames": min(nf, MAX_FRAMES_GUARD),
            "width": w, "height": h, "duration_seconds": nf / fps if fps > 0 else 0.0,
            "is_landscape": w > h}
...
def process_video(input_path, output_path, target_preset_label="Match source (no upscale)", tracking_mode="subject",
                  ...
                  smooth_window=15, adaptive_smoothing=True, rule_of_thirds=True, scene_cut_threshold=0.35,
    ...
    info = get_video_info(input_path)
    fps, total_frames, orig_w, orig_h, duration = info["fps"], info["total_frames"], info["width"], info["height"], info["duration_seconds"]
    ...
    prev_gray, prev_flow = None, None; frame_idx = 0; report_n = max(1, total_frames//25); det_end = 0.42
    cap = cv2.VideoCapture(input_path)
    while frame_idx < total_frames:
        ret, frame = cap.read()
        if not ret: break
        curr_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        if is_scene_change(prev_gray, curr_gray, scene_cut_threshold): scene_cuts.append(frame_idx); prev_flow = None
        prev_gray = curr_gray
        if frame_idx%sample_interval == 0:
            ...
            if tracking_mode == "talking_head":
                ...
                if det is not None: center = frame_for_union(det.ux1, det.uy1, det.ux2, det.uy2, orig_w, orig_h, crop_w, crop_h)
                elif use_optical_flow:
                    small = cv2.resize(curr_gray, (orig_w//2, orig_h//2))
                    if prev_flow is not None:
                        fc = optical_flow_center(prev_flow, small, orig_w//2, orig_h//2)
    ...
    if rule_of_thirds and tracking_mode != "talking_head":
        speeds = _compute_speeds(all_centers, smooth=3); vel_vecs = _compute_vel_vecs(all_centers, look=3)
        framed = [apply_framing_bias(cx, cy, vx, vy, speeds[i], orig_w, orig_h, crop_w, crop_h) for i, ((cx, cy), (vx, vy)) in enumerate(zip(all_centers, vel_vecs))]
        all_centers = framed
    elif tracking_mode == "talking_head" and rule_of_thirds:
        hw2, hh2 = crop_w//2, crop_h//2; framed = []
        for cx, cy in all_centers:
            tx = min([orig_w//3, 2*orig_w//3], key=lambda x: abs(x-cx)); nx = int(cx+0.15*(tx-cx)); nx = max(hw2, min(nx, orig_w-hw2)); framed.append((nx, cy))
        all_centers = framed
    hw, hh = crop_w//2, crop_h//2
    ...
    temp_mp4, temp_final = None, None
    try:
        fd, temp_mp4 = tempfile.mkstemp(suffix=".mp4"); os.close(fd)
        # FIXED: Use MP4 container & H.264 codec for FFmpeg compatibility
        ...
        cap = cv2.VideoCapture(input_path); rpt_n = max(1, total_frames//40)
        ...
        if not os.path.exists(temp_mp4) or os.path.getsize(temp_mp4)<1000: raise ProcessingError("Rendered MP4 is empty.")

        _p(0.87, "🎵 Encoding…" + (" Burning subtitles…" if srt_path else ""))
        fd, temp_final = tempfile.mkstemp(suffix=".mp4"); os.close(fd)
        style = SUBTITLE_STYLES.get(subtitle_style_name, SUBTITLE_STYLES["Bold White (TikTok)"])
        _ffmpeg_encode(temp_mp4, input_path if _has_audio(input_path) else None, temp_final,
        ...
        if not os.path.exists(temp_final) or os.path.getsize(temp_final)<1000: raise ProcessingError("FFmpeg produced empty output.")
        shutil.move(temp_final, output_path); temp_final = None
        _p(1.0, "✅ Done!")
...
def process_clips_batch(input_path, output_dir, clips, target_preset_label="720p   (720×1280  — HD)", tracking_mode="subject",
                        talking_head_bias=0.30, confidence=0.45, smooth_window=15, adaptive_smoothing=True, rule_of_thirds=True,
                        crf=23, encoder_preset="fast", audio_bitrate="128k", yolo_weights="yolov8n.pt", burn_subtitles=False,
                        whisper_model="base", subtitle_style_name="Bold White (TikTok)", subtitle_max_chars=42, progress_callback=None):
    def _p(v, msg=""):
        if progress_callback:
            try: progress_callback(v, msg)
            except: pass
    ...
    for i, clip in enumerate(clips):
        ...
        try:
            ...
            meta = process_video(trimmed_path, out_path, target_preset_label=target_preset_label, tracking_mode=tracking_mode,
                                 ...
                                 adaptive_smoothing=adaptive_smoothing, rule_of_thirds=rule_of_thirds, crf=crf,
                                 encoder_preset=encoder_preset, audio_bitrate=audio_bitrate, yolo_weights=yolo_weights,
                                 burn_subtitles=burn_subtitles, whisper_model=whisper_model,
                                 subtitle_style_name=subtitle_style_name, subtitle_max_chars=subtitle_max_chars, progress_callback=clip_cb)
