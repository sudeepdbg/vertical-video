...
def _ffmpeg_encode(video_path: str, audio_source: Optional[str], output_path: str,
    ...
    # FIX: Add robust input flags for reading intermediate files
    cmd = ["ffmpeg", "-y",
           "-fflags", "+genpts",
           "-probesize", "50M",
           "-analyzeduration", "10M",
...
def get_video_info(path: str) -> Dict[str, Any]:
    ...
    return {"fps": fps, "total_frames": total_frames, "width": w, "height": h,
            "duration_seconds": duration, "is_landscape": w > h}
...
def detect_faces(frame: np.ndarray, confidence_thresh: float = 0.6) -> List[Tuple[int, int, int, int]]:
    ...
    raw = haar.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5,
...
def _compute_speeds(centers, smooth=9):
    ...
    raw = [0.0] + [float(np.sqrt((centers[i][0]-centers[i-1][0])**2 + 
                                  (centers[i][1]-centers[i-1][1])**2)) for i in range(1, n)]
    w = min(smooth, n)
...
def _compute_vel_vecs(centers, look=6):
    ...
    out = []
    for i in range(n):
        j, k = min(i+look, n-1), max(i-look, 0)
        span = j-k
        if span > 0:
            out.append(((centers[j][0]-centers[k][0])/span, (centers[j][1]-centers[k][1])/span))
        else:
            out.append((0.0, 0.0))
    return out
    ...
    def _p(v, msg=""):
        if progress_callback:
            try:
                ...
                pass
    scores, scene_cuts = [], []
    prev_gray, prev_frame = None, None
    frame_idx = 0
...
def detect_clips(input_path, min_duration_sec=25.0, max_duration_sec=65.0, target_n_clips=10,
    ...
    scores, scene_cuts_frames = _compute_frame_scores(input_path, fps, total_frames, 
    ...
    for ci, (start_sec, end_sec, score) in enumerate(clip_candidates):
        ...
        cap.release()
        if soi_xs:
            soi_region = _soi_region_label(int(np.median(soi_xs)), int(np.median(soi_ys)), orig_w, orig_h)
        mins_s, secs_s = int(start_sec//60), int(start_sec%60)
        mins_e, secs_e = int(end_sec//60), int(end_sec%60)
        title = f"Clip {ci+1} ({mins_s}:{secs_s:02d} – {mins_e}:{secs_e:02d})"
        segments.append(ClipSegment(start_sec, end_sec, score, soi_region, 
                                    int(sample_times[len(sample_times)//2]*fps), title))
...
def process_video(input_path, output_path, target_preset_label="Match source (no upscale)", 
                  tracking_mode="subject", talking_head_bias=0.30, sample_interval=None, 
                  confidence=0.45, use_optical_flow=True, smooth_window=27, adaptive_smoothing=True, 
                  rule_of_thirds=True, scene_cut_threshold=0.35, output_fps=None, crf=23, 
                  encoder_preset="fast", audio_bitrate="128k", yolo_weights="yolov8n.pt", 
                  burn_subtitles=False, whisper_model="base", whisper_language=None, 
                  subtitle_style_name="Bold White (TikTok)", subtitle_max_chars=42, 
                  subtitle_translate_to=None, progress_callback=None):
    def _p(v, msg=""):
        if progress_callback:
            try:
    ...
    while frame_idx < total_frames:
        ...
        if frame_idx%sample_interval == 0:
            ...
            if tracking_mode == "talking_head":
                ...
                if faces:
                    center = talking_head_center(faces, orig_w, orig_h, crop_w, crop_h, talking_head_bias)
                    det_dropout_count = 0
                ...
                if det is not None:
                    center = frame_for_union(det.ux1, det.uy1, det.ux2, det.uy2, 
    ...
    if rule_of_thirds and tracking_mode != "talking_head":
        ...
        biased = [apply_framing_bias(cx, cy, vx, vy, speeds[i], orig_w, orig_h, crop_w, crop_h)
    ...
    temp_final = None
    try:
        # FIX: Use MP4 container with H.264 codec instead of AVI/MJPEG
...
def process_clips_batch(input_path, output_dir, clips, target_preset_label="720p   (720×1280  — HD)", 
                        tracking_mode="subject", talking_head_bias=0.30, confidence=0.45, 
                        smooth_window=27, adaptive_smoothing=True, use_optical_flow=True, 
                        rule_of_thirds=True, crf=23, encoder_preset="fast", audio_bitrate="128k", 
